...
pair<double, State> run(string name) {
    ...
    State res;
    if (name=="BFS") res = bfs();
    else if (name=="DFS") res = dfs();
    else if (name=="UCS") res = ucs();
    else if (name=="Greedy") res = greedy();
    else res = astar();
    ...
    for (int i=0;i<algos.size();i++) {
        ...
        for (int v=0;v<res.routes.size();v++) {
            ...
            for (int j=0;j<res.routes[v].size();j++) {
                out << res.routes[v][j];
                if (j+1<res.routes[v].size()) out << ",";
            }
            out << "]";
            if (v+1<res.routes.size()) out << ",";
        }

        out << "]\n}";

        if (i+1<algos.size()) out << ",\n";
    }
